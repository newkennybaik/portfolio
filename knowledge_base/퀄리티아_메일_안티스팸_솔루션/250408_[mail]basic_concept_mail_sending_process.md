...
- 메일은 일반적으로 송신자 서버의 메일세션(MTA)과 수신자 서버의 메일세션(MTA) 2개의 로그로 구성됨.
- 송신자 서버의 메일세션에서 끊겼다면 거의 메일서버 네트워크나 프로그램에 장애가 있는 상황일 것이고,
- 수신자 서버로 메일은 전달이 되었지만 수신서버가 끊은 경우는 대부분 수신서버의 정책 문제이기 때문임.
