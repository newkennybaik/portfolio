...
### 2.1 스팸차단 솔루션 (WBlock)
- 퀄리티아의 스팸차단 솔루션인 WBlock은 메일을 대신 받아서 스팸을 필터링한 후 전달해주는 구조임. 
- 메일서버끼리 다이렉트로 통신하는 것이 아닌 SMTP 통신을 메일서버 대신하고, 필터링한 다음 안전한 메일을 다시 SMTP 통신으로 내부 메일서버로 전달해주는 구조임. 
- SMTP Relay라고 하며, 전달 구조는 아래와 같음.

```bash
┌──────────────────────────────┐
│송신자의 메일 클라이언트(MUA)   │
└──────────────────────────────┘
-> 네트워크/DNS 조회를 통한 SMTP 통신 (MTA <> MTA) <- SMTP 통신중 스팸 필터도 동작함
┌──────────────────────────────┐
│스팸서버 Relay                 │
└──────────────────────────────┘
-> 라우팅 혹은 내부 네트워크/DNS 조회를 통한 SMTP 통신 (MTA <> MTA)
┌──────────────────────────────┐
│수신자의 메일 클라이언트(MUA)   │
└──────────────────────────────┘
```
- 스팸서버도 SMTP서버, 메일서버도 SMTP서버임.
- 지란지교의 SNIPER라던지 안랩의 MDS도 비슷한 전달구조를 갖고 있음. 
- 전에 일했던 네이버웍스도 라우팅 기능이 있어서 relay 서버로 구성할 수 있음.
...
- 퀄리티아의 메일제품은 앞단에 WBlock이라는 스팸장비가 받아서 메일서버에 전달해주므로 SMTP 세션에 전달내역이 총 2개가 있음. 따라서 두 구간에 전달되는 흐름을 보고 사유를 분석해야함.
...
- 앞단에 WBlock이라는 스팸장비가 받아서 메일서버에 전달해주므로 SMTP 세션에 전달내역이 총 2개가 있음. 
- 세션하나에 송신자 서버의 발송내역(EHLO)과 수신자 서버에서의 수신내역(EHLO) 2개의 로그로 구성됨.
- 송신자 서버의 발송내역에서 끊겼다면 송신자의 메일서버 네트워크나 프로그램에 장애가 있는 상황일 것이고,
- 발송내역에는 250을 반환했는데 메일이 전달이 안된 사례는 수신서버의 사유인 경우가 많음.
- 아래 메일케이스를 보면, 발송내역에 스팸서버에서 네이버 서버로 정상적으로 전달이 되었으나 네이버의 수신내역에서 메일 패킷을 끊어버린 것을 볼 수 있음.
...
## 6. 정상적으로 수신 된 메일 사례
- 퀄리티아의 WBlock 제품은 메일을 대신 수신받으면서 SMTP 필터링이 동작함.
...
4. 기업솔루션들은 이런식으로 서버 by 서버로 SMTP를 다이렉트 통신하는 것이 아닌 경유하고 또 경유해서 내부로 전달해서 보안을 챙기는 경우가 많음. (TLS도 씀)