...
- 고객사에서 메일이 정상적으로 송/수신이 안된 사례에 대한 문의가 빗발치는 경우가 많고, 아래와 같이 로그 분석을 통해서 정상적으로 수신이 안된 사유를 확인할 수 있음.

### 5.1 퀄리티아의 WBlock 스팸서버에서 텔넷 명령어로 발송 시 발송 불가 사유 분석
- 아까 위에 WBlock 스팸서버에서 newkennybaik@naver.com 네이버 사용자에게 메일을 발송했으나 수신되지 못했음.
...
2. 세션 ID 검색. 송신자 서버의 메일세션(MTA)과 수신자 서버의 메일세션(MTA) 2개의 로그로 구성됨
- 메일이 수신이 안되더라도, 송신서버에서 끊기는것과 수신서버에서 끊기는것에는 엄청난 차이가 있음. 
- 왜냐하면 수신서버로 메일이 전달이 되었는지 여부가 네트워크 문제인지, 정책 문제인지 판단 할 수 있기 때문임.
...
수신 서버인 네이버까지는 도달했으나, 네이버의 정책상 송신자의 인증부족(네이버 계정으로 로그인 안함)으로 끊어버림
